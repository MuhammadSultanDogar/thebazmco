...
function getSessionSecret(): string {
  ...
  if (process.env.NODE_ENV === "production") {
    throw new Error("MANAGER_SESSION_SECRET is required in production")
  }
...
export function getManagerCredentials() {
  return {
    username: process.env.MANAGER_USERNAME ?? "hassan",
    password: process.env.MANAGER_PASSWORD ?? "chotakela1",
  }
}
